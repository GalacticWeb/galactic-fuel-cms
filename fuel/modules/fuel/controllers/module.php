...
class Module extends Fuel_base_controller {
		...
		if (is_ajax())
		{
			...
			if ($has_delete_permission)
			{
				$delete_func .= 'if (isset($cols[$CI->model->key_field()]))
				{
					...
					$link = "<a href=\"".$url."\" class=\"action_delete label label-danger\">".lang("table_action_delete")."</a>";
					$link .= " <input type=\"checkbox\" name=\"delete[".$cols[$CI->model->key_field()]."]\" value=\"1\" id=\"delete_".$cols[$CI->model->key_field()]."\" class=\"multi_delete\"/>";
				}';				
			}
			...
			foreach($this->table_actions as $key => $val)
			{
				...
				else if (strtoupper($val) == 'EDIT')
				{
					if ($this->fuel->auth->has_permission($this->permission, "edit"))
					{
						...
						$this->data_table->add_action(lang('table_action_edit'), $action_url, 'url', array('class' => 'action_edit label label-primary'));
					}
				}
				...
				else
				{
					if (strtoupper($val) != 'VIEW' OR ( ! empty($this->preview_path) AND strtoupper($val) == 'VIEW'))
					{
						...
						$this->data_table->add_action($action_name, $action_url, 'url', array('class' => 'action_view label label-info'));
					}
				}
			}
		...
		if ($this->input->post($this->model->key_field()))
		{
			if ($this->_process_edit($id) AND !has_errors())
			{
				...
				else
				{
					...
					// Redirect back to the overview page instead of the form view...
	...
	function life_edit($id = NULL, $field = NULL) {
	 ...
	 * @return	void
	 */	
	function inline_edit($id = NULL, $field = NULL, $life_edit = FALSE)
	{
		if (empty($id))
		{
			show_404();
		...
		if( $life_edit ) $this->life_edit($id, $field);
		else $this->edit($id, $field);
	}
