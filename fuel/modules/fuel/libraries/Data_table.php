...
class Data_table {
	...
	public $css_class = 'data table table-condensed table-hover'; // css class to be used with the form
	...
	public function add_row($columns = array(), $attrs = array(), $index = NULL, $action = NULL)
	{
		...
		if (is_array($columns))
		{
			...
			foreach($columns as $key => $val)
			{
				...
				if ($i == 0 AND !empty($this->_actions) AND $this->actions_field == 'first') 
				{
					$fields[] = new Data_table_field('actions', $this->_render_actions($this->_actions, $columns), array('class' => 'actions hidden-xs'));
					$i++;
				}
				// add the actions
