		...
		foreach($this->_data as $file)
		{
					...
					))
			{
				...
				if (!empty($params['resize_and_crop']) OR (!empty($params['resize_method']) AND $params['resize_method'] == 'resize_and_crop'))
				{
					...
					$thumb_filename = basename($file['file_name'], $file['file_ext']).'_thumb'.$file['file_ext'];
					if ( file_exists('assets/images/'.$thumb_filename) &&  file_exists('assets/images/thumbs') ) {
						rename('assets/images/'.$thumb_filename, 'assets/images/thumbs/'.$thumb_filename);
					}
				}
				else
				{
