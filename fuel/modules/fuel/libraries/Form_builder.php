...
class Form_builder {
	...
	public $css_class = 'form-horizontal'; // css class to be used with the form
	...
	public $tooltip_format = '<span title="{?}" data-toggle="tooltip" data-placement="right">[?]</span>'; // tooltip formatting string
	...
	public $render_format = 'divs'; // default render format
    ...
    public $div_field_input_class = 'form-control';
    public $fieldset_class = 'tab-content tab-pane col-xs-12';
	...
	public function render_divs($fields = NULL)
	{
		...
		foreach($this->_fields as $key => $val)
		{
			...
			else if ((is_array($val['name']) AND in_array($val['name'], $this->displayonly)) OR  $val['displayonly'] OR  (is_string($this->displayonly) AND strtolower($this->displayonly) == 'all'))
			{
				...
				$str .= "<div".$this->_open_row_attrs($val).'>';
				$str .= "<span class=\"col-xs-12 col-md-1 control-label\">";
				$str .= $val['before_label'].$this->create_label($val, FALSE).$val['after_label'];
				$str .= "</span>";
				$str .= "<span class=\"field col-xs-12 col-md-11\">";
	...
	protected function _render_actions()
	{
		...
		if ( ! empty($this->reset_value))
		{
			...
			{
				$str .= $this->form->reset($this->reset_value, '', array('class' => 'btn btn-default reset'));
			}
		}
		...
		if ( ! empty($this->cancel_value))
		{
			...
			{
				$cancel_attrs = array('class' => 'btn btn-danger cancel');
		...
		if (!empty($this->submit_value) AND $this->displayonly != 'all')
		{
			...
			else
			{
				...
				// }
				$str .= $this->form->$submit_btn($this->submit_value, $submit_name, array('class' => 'btn btn-success submit', 'id' => $submit_id));
			}
		}
	...
	public function create_text($params)
	{
		...
		$attrs = array(
			'id' => $params['id'],
			'class' => $params['class'].' '.$this->div_field_input_class, 
	...
	public function create_select($params)
	{
		...
		$attrs = array(
			'id' => $params['id'],
			'class' => $params['class']. ' '. $this->select_class, 
			'readonly' => $params['readonly'], 
			'disabled' => $params['disabled'],
			'required' => (!empty($params['required']) ? TRUE : NULL),
			'data' => $params['data'],
	...
	public function create_fieldset($params)
	{
		$params = $this->normalize_params($params);
		$attrs = array(
			'class' => $params['class'].' '.$this->fieldset_class, 
		);
		$str = '';
