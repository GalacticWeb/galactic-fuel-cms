...
class Fuel_assets_model extends CI_Model {
	...
	public $thumbnail_folder = 'thumbs';
	...
	public function list_items($limit = null, $offset = 0, $col = 'name', $order = 'asc')
	{
		...
		for ($i = 0; $i < $cnt; $i++)
		{
			if (!empty($tmpfiles[$i]) AND !empty($files[$tmpfiles[$i]]))
			{
					...
					(stripos($files[$key]['name'], $this->filters['name']) !== FALSE || stripos($key, $this->filters['name']) !== FALSE)))
				{
					...
					$file['name'] = $key;
					$file['preview/kb'] = round( ($files[$key]['size'] / 1024), 1 );
					$file['link'] = NULL;
		...
		$thumbnail_folder = $this->thumbnail_folder;

		// after sorting add the image thumbnails
		foreach ($return as $key => $val)
		{
			if (is_image_file($return[$key]['name']))
			{
				$ext = pathinfo($return[$key]['name'], PATHINFO_EXTENSION);
				$thumb_filename = rtrim( basename($return[$key]['name'], $ext), "." ).'_thumb.'.$ext;

				//$this->get_file_type($return[$key]['name']);

				// check if thumbnail existsm --> show it
				if ( file_exists($_SERVER['DOCUMENT_ROOT'].$asset_type_path.$thumbnail_folder.'/'.$thumb_filename) ) {
					$return[$key]['preview/kb'] = $return[$key]['preview/kb'].' kb <div><a href="'.$asset_type_path.$return[$key]['name'].'" target="_blank" data-lightbox="'.$return[$key]['name'].'"><img src="'.$asset_type_path.$thumbnail_folder.'/'.($thumb_filename).'?c='.time().'" class="img-responsive img_crop" border="0"></a></div>';
				}
				// ... otherwise show original picture
				else {
					$return[$key]['preview/kb'] = $return[$key]['preview/kb'].' kb <div><a href="'.$asset_type_path.$return[$key]['name'].'" target="_blank" data-lightbox="'.$return[$key]['name'].'"><img src="'.$asset_type_path.($return[$key]['name']).'?c='.time().'" class="img-responsive img_crop" border="0"></a></div>';
				}
	...
	/**
	 * Returns the thumb-name of a file
	 *
	 * @access	public
	 * @param	string	An asset file
	 * @return	string
	...
	public function get_thumb_name($file)
	{
		...
		return rtrim( basename($filename, $ext), "." ).'_thumb.'.$ext;
	}
	
	// --------------------------------------------------------------------

	/**
	 * Checks if the file is of type thumb or regular
	 *
	 * @access	public
	 * @param	string	An asset file
	 * @return	array
	...
	public function get_file_type($file)
	{
		...
		$ext = $this->get_extension($file);
		$filename = rtrim( basename($file, $ext), "." );

		if ( ($pos = strrpos($filename, "_")) !== FALSE ) { 
    		$thumb_test = substr($filename, $pos+1);
    		
    		if ( $thumb_test == 'thumb' ) {
    			$filetype['thumb'] = $filename.'.'.$ext;
    			$filetype['original'] = substr($filename, 0, $pos).'.'.$ext; 
    			$filetype['type'] = 'thumb'; 

				return $filetype;
    		}
		}	
		$filetype['thumb'] = $filename.'_thumb.'.$ext;
		$filetype['original'] = $filename.'.'.$ext;
	...
	protected function _delete($file)
	{
		...
		// check for thumbnail
		/*if( $filetype['type'] == 'thumb' ) {
			$thumbs_folder = WEB_ROOT.$CI->config->item('assets_path').$this->thumbnail_folder.'/';
			$file = trim(str_replace($thumbs_folder, '', $file), '/');
			
			// normalize file path
			$filepath_thumb = $thumbs_folder.$file;
			$parent_folder = WEB_ROOT.$CI->config->item('assets_path');;

			$filepath_original = $parent_folder.$file;
			
			if (file_exists($filepath_thumb)) {
				$deleted = unlink($filepath_thumb);
			}
			if (file_exists($filepath_original)) {
				$deleted = unlink($filepath_original);
			}
		}
		else {*/
			...
			if (file_exists($filepath)) {
				...
				if ( is_image_file($filetype['original']) ) {

					$thumb_path = $assets_folder.'images/'.$this->thumbnail_folder.'/'.$filetype['thumb'];

					if ( file_exists($thumb_path) ) {
						$deleted = unlink($thumb_path);
					}
				}
			}
	...
	public function form_fields($values = array())
	{
		...
		//$fields['dropfile'] = array('label' => lang('form_label_file_drop'), 'type' => 'file', 'class' => 'multifile', 'accept' => $accepts); 
		$fields['userfile'] = array('label' => lang('form_label_file'), 'type' => 'file','data' => array('form-action-path' => 'xxx'), 'class' => 'multifile', 'accept' => $accepts); // key is userfile because that is what CI looks for in Upload Class
		...
		$fields['create_thumb'] = array('label' => lang('form_label_create_thumb'), 'type' => 'checkbox', 'comment' => lang('assets_comment_thumb'), 'checked' => true, 'value' => '1');
		...
		$fields['width'] = array('label' => lang('form_label_width'), 'comment' => lang('assets_comment_width'), 'size' => '3', 'value' => 100);
		$fields['height'] = array('label' => lang('form_label_height'), 'comment' => lang('assets_comment_height'), 'size' => '3', 'value' => 100);
		$fields['master_dim'] = array('type' => 'select', 'label' => lang('form_label_master_dim'), 'options' => array('height' => 'height', 'width' => 'width', 'auto' => 'auto'), 'comment' => lang('assets_comment_master_dim'));
