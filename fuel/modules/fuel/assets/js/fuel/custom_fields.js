...
function set_file_action_url(url) {
...
fuel.fields.wysiwyg_field = function(context){
	...
	var createPreview = function(id){
		...
		var previewButton = '<a href="#" id="' + id + '_preview" class="btn_field editor_preview btn btn-default">' + fuel.lang('btn_preview') + '</a>';
...
fuel.fields.file_upload_field = function(context){
	...
	$(document).ready(function() {
		...
		// URL from form action attribute
...
fuel.fields.asset_field = function(context, options){
	...
	var showAssetsSelect = function(){
		...
		var html = '<iframe src="' + url +'" id="asset_inline_iframe" class="inline_iframe" frameborder="0" width="99.6%"></iframe>';
	...
	$('.asset_select', context).each(function(i){
		if ($(this).parent().find('.asset_upload_button').length == 0){
			...
			$(this).after('&nbsp;<a href="'+ jqx_config.fuelPath + '/assets/select/' + assetFolder + '" class="btn_field asset_select_button btn btn-primary ' + assetFolder + '" data-folder="' + assetFolder + '">' + fuel.lang('btn_select') + ' ' + btnLabel + '</a>');
		}
	});
	...
	var showAssetUpload = function(url){
		var html = '<iframe src="' + url +'" id="add_edit_inline_iframe" class="inline_iframe" frameborder="0" width="99.6"></iframe>';
		$modal = fuel.modalWindow(html, 'inline_edit_modal', true);
		// // bind listener here because iframe gets removed on close so we can't grab the id value on close
	...
	$('.asset_upload', context).each(function(i){
		if ($(this).parent().find('.asset_upload_button').length == 0){
			...
			$(this).after('&nbsp;<a href="'+ jqx_config.fuelPath + '/assets/inline_create/" class="btn_field asset_upload_button btn btn-default ' + assetFolder + '" data-params="' + $(this).attr('data-params') + '">' + btnLabel + '</a>');
		}
	});
	...
	var refreshImage = function(activeField){
		...
		// preview crop set?
		var preview_crop = $preview.data('previewcrop');
		if ( preview_crop == 1 ) {
			var preview_crop_width = $preview.data('previewcrop-width');
			var preview_crop_height = $preview.data('previewcrop-height');
		} else {
			var preview_crop_width = false;
			var preview_crop_height = false;
		}
		...
		$.each(imgValues, function(img){
			...
			if (this.length && this.toLowerCase().match(/\.jpg$|\.jpeg$|\.gif$|\.png$/)){
				...
				if ( preview_crop == 1 ) {
					previewHTML += '<div class="preview-crop-wrap">';
					previewHTML += '<img class="img-thumbnail img-thumb-preview img-preview-crop" src="' + newSrc + '" style="' + imgStyles + '">';
					previewHTML += '</div>';
				} else {
					previewHTML += '<a href="' + newSrc + '" target="_blank" data-lightbox="'+ newSrc +'">';
					previewHTML += '<img class="img-thumbnail img-thumb-preview" src="' + newSrc + '" style="' + imgStyles + '">';
					previewHTML += '</a>';
				}
		...
		// init crop functionality -------------------------------
		if ( preview_crop == 1 ) {
			initCropping('.img-preview-crop', preview_crop_width, preview_crop_height);
		}
	...
	// init crop functionality
	var initCropping = function(activeField, w, h){
		var picture = $(activeField);
      	...
      	picture.on('load', function(){
			...
			$('input#main_image_crop_width').val( data.w );
			$('input#main_image_crop_height').val( data.h );
			$('input#main_image_crop_x').val( data.x );
			$('input#main_image_crop_y').val( data.y );

			// set hidden input fields on change
...
fuel.fields.inline_edit_field = function(context){
	...
	$('.add_edit', context).each(function(i){
		...
		var btnClasses = ($field.attr('multiple')) ? 'btn_field btn_field_right btn btn-default ' : 'btn_field btn btn-default ';
...
fuel.fields.url_field = function(context, options){
	...
	var showUrlSelect = function(){
		...
		var html = '<iframe src="' + url +'" id="url_inline_iframe" class="inline_iframe" frameborder="0" width="99.6%"></iframe>';
		$modal = fuel.modalWindow(html, 'inline_edit_modal', true);
