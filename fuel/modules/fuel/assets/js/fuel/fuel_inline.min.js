        ...
        timeout_id = window[str_setTimeout](function() {
            elems.each(function() {
                var elem = $(this),
                    ...
                    height = elem.height(),
                    data = $.data(this, str_data);
                if (data && (width !== data.w || height !== data.h)) elem.trigger(str_resize, [data.w = width, data.h = height])
            });
            loopy()
        }, jq_resize[str_delay])
    }
...
fuel.modalWindow = function(html, cssClass, autoResize, onLoadCallback, onCloseCallback) {
    ...
    else $("#" + modalId, $context).html('<div class="loader"></div><a href="#" class="modal_close jqmClose"></a><div class="modal_content"></div>');
...
(function($) {
    ...
    var X_OFFSET = 16;
    var Y_OFFSET = 16;
    var editorsOn = parseInt($.supercookie("fuel_bar",
        "show_editable_areas")) == 1;
    ...
    jQuery.resize.delay = fuel.resizeDelay ? fuel.resizeDelay : 1E3;
    ...
    $(document).ready(function() {
        ...
        function init() {
            ...
            fuel.modalWindow = function(html, cssClass, callback) {
                ...
                $modal.find("iframe").load(function() {
                    ...
                    docHeight = actionsHeight + notificationsHeight + mainContentHeight + listTableHeight + 30;
            ...
            fuel.setIframeSize = function(iframe) {
                ...
                if ($("#fuel_main_content_inner .form, #fuel_actions", contentDoc).size()) {
                    var width1 = $("#fuel_main_content_inner .form", contentDoc).outerWidth(false) + 74;
                ...
                } else if ($("#login", contentDoc).size()) docWidth = $("#login", contentDoc).width();
                else docWidth = $(contentDoc).width();
                if (docHeight == 0) docHeight = $(contentDoc).height();
                $(iframe).height(docHeight);
                $(iframe).width(docWidth)
            }
        }
            ...
            if (markers.size() > 0) {
                ...
                markers.each(function(i) {
                    ...
                    if (module == "pagevariables" && pageId != 0 || module != "pagevariables") {
                        ...
                        var newClass =
                            $this.attr("data-rel").substr(0, 6) == "create" ? " __fuel_edit_marker_new__" : "";
        ...
        function refresh() {
            if (editorsOn) {
                moveMarkers();
                if (activeEditor) var iframe = activeEditor.find("iframe")[0]
            }
        }
        ...
        function moveMarkers() {
            ...
            markers.each(function(i) {
                ...
                if ($this.filter(":hidden").size() != 0) $("#__fuel_edit__" + i).hide();
                else $("#__fuel_edit__" + i).show()
            });
        ...
        function adjustPosition(editors, $obj, counter) {
            editors.each(function(i) {
                ...
                if (counter <= maxAdjustLoops && $obj.attr("id") != $compareObj.attr("id") && Math.abs(topPos - parseInt($compareObj.css("top"))) < Y_OFFSET && Math.abs(leftPos -
                        parseInt($compareObj.css("left"))) < X_OFFSET) {
        ...
        function initEditors() {
            ...
            var closeEditor = function() {
                if (activeEditor) {
                    ...
                    var contentDoc = iframe.contentDocument;
                    if ($(".success", contentDoc).size()) top.window.location.reload();
                    else {
            ...
            var ajaxSubmit = function($form) {
                $form.attr("action", formAction).ajaxSubmit(function(html) {
                    if ($(html).is("error")) {
                        var msg = $(html).html();
                        if (msg != "" || msg != "1") {
                            $form.find(".inline_errors").html(msg).animate({
            ...
            editors.each(function(i) {
                var $this = $(this);
                var module = $this.attr("data-module");
                var _anchor = $(".__fuel_edit_marker__",
                    this);
                _anchor.mouseover(function() {
                    $(".__fuel_edit_marker_inner__", this).stop().css(resetCss).show()
                });
                _anchor.mouseout(function() {
                    if (activeEditor && activeEditor.attr("title") == $this.attr("title")) return;
                    else $(".__fuel_edit_marker_inner__", this).stop().css(resetCss).hide()
                });
                _anchor.click(function(e) {
                    if (!activeEditor || activeEditor != $this) {
                        if ($(".__fuel_edit_form__", $this).children().not("img").size() == 0) {
                            ...
                            var param1 = relArr[0];
                            if (module == "pagevariables") var param2 =
                                pageId;
                            else var param2 = relArr.length >= 2 ? relArr[1] : "";
                            if (param1.substr(0, 6) == "create") {
                                var qString = param2.length ? "?" + param2 : param1;
                                var url = $(this).attr("href") + qString
                            } else var url = $(this).attr("href") + param1 + "/" + param2;
                            var lang = $("#__fuel_language__").val();
                            if (lang && lang.length) url = url + "?lang=" + $("#__fuel_language__").val();
                            if (_anchor.next(".__fuel_edit_form__").find("iframe").size() == 0) {
                                var iframeId = "__fuel_iframe__" + $this.attr("id");
                                _anchor.next(".__fuel_edit_form__").html('<div class="loader"></div><iframe src="' +
                                    url + '" id="' + iframeId + '" frameborder="0" scrolling="no" class="inline_iframe"></iframe>');
                        ...
                        } else _anchor.next(".__fuel_edit_form__").show();
                        ...
                        if (activeEditor && activeEditor.attr("title") != $this.attr("title")) closeEditor();
                        activeEditor = $this
                    } else closeEditor();
                    return false
                })
        ...
        function initFUELBar() {
            var hideEditors = function() {
                ...
                $.supercookie("fuel_bar", "show_editable_areas", "0", {
                    path: cookiePath
                })
            };
            var showEditors = function() {
                ...
                $.supercookie("fuel_bar", "show_editable_areas", "1", {
                    path: cookiePath
                })
            };
            var toggleEditors = function(shown) {
                if (shown) hideEditors();
                else showEditors()
            };
            $("#__fuel_page_edit_toggle__").click(function() {
            ...
            $("#__fuel_language__").change(function() {
                ...
                if ($("#__fuel_language_mode__").val() == "segment")
                    if ($("#__fuel_language_default__").val() != lang) var url = basePath + lang + "/" + pageLocation;
                    else var url = basePath + pageLocation;
                else {
            ...
            var hideFuelBar = function(animate) {
                ...
                if (animate) $("#__fuel_edit_bar__").animate({
                    right: "-" + barHideX + "px"
                }, 500);
                else $("#__fuel_edit_bar__").css({
                    right: "-" + barHideX + "px"
                });
                fuelBarOn = false;
                elem.parent("li").removeClass("active");
                $.supercookie("fuel_bar", "show_fuel_bar", "0", {
                    path: cookiePath
                })
            };
            var showFuelBar =
                function(animate) {
                    var elem = $("#__fuel_page_toolbar_toggle__");
                    if (animate) $("#__fuel_edit_bar__").show().animate({
                        right: "0px"
                    }, 500);
                    else $("#__fuel_edit_bar__").show().css({
                        right: "0px"
                    });
                    $(".__fuel_edit_bar__").width();
                    fuelBarOn = true;
                    elem.parent("li").addClass("active");
                    $.supercookie("fuel_bar", "show_fuel_bar", "1", {
                        path: cookiePath
                    })
                };
            $("#__fuel_page_toolbar_toggle__").click(function() {
                toggleFuelBar(fuelBarOn, true);
                return false
            });
            var toggleFuelBar = function(shown, animate) {
                if (shown) hideFuelBar(animate);
                else showFuelBar(animate)
            };
